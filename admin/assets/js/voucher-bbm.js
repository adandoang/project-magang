...
(function () {
    ...
    async function loadRequests(forceRefresh = false) {
        if (!forceRefresh) {
            ...
            if (cached) { masterRequests = cached; applyRequestFilter(); updateStats(); showCacheIndicator(); return; }
        }
        ...
        try {
            ...
            masterRequests = Array.isArray(res) ? res : [];
    ...
    window.bbmViewDetail = (id) => {
        ...
        if (el) el.innerHTML = `<div class="info-grid">
    ...
    // ═══ TAB 2: CATATAN KETERLAMBATAN ════════════════════════
    async function loadViolations(forceRefresh = false) {
        if (!forceRefresh) {
            const cached = getFromCache(CACHE_KEYS.VIOLATIONS);
            if (cached) { masterViolations = cached; applyViolationFilter(); showCacheIndicator(); return; }
        }
        ...
        try {
            ...
            masterViolations = Array.isArray(res) ? res : [];
    ...
    function renderPaginatedViolations() {
        ...
        if (allViolations.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:40px;color:#94a3b8;">Tidak ada catatan keterlambatan</td></tr>';
            if (cards) cards.innerHTML = '<div style="text-align:center;padding:2rem;color:#64748b;">Data tidak ditemukan</div>';
            if (pgn) pgn.innerHTML = ''; return;
        }
        ...
        tbody.innerHTML = items.map((v, idx) => {
            ...
            return `<tr>
                <td>${v.bulan || '-'}</td>
                <td>${v.unit || '-'}</td>
                <td>${v.tanggal_pengambilan || '-'}</td>
                <td>${v.tanggal_pengembalian || '-'}</td>
                <td style="font-family:monospace;">${v.nomor_kendaraan || '-'}</td>
                <td>
                    <div class="action-buttons">
                        <div class="btn-icon-group">
    ...
    window.bbmSubmitViol = async () => {
        const bulan = document.getElementById('bbm-viol-bulan')?.value.trim();
        const unit = document.getElementById('bbm-viol-unit')?.value.trim();
        const tglAmbil = document.getElementById('bbm-viol-tgl-ambil')?.value.trim();
        if (!bulan || !unit || !tglAmbil) { if (window.showToast) showToast('Bulan, unit, dan tanggal pengambilan harus diisi', 'error'); return; }
        const btn = document.getElementById('bbm-submit-viol-btn');
        const orig = btn.innerHTML; btn.disabled = true; btn.innerHTML = '<span class="spinner spinner-sm"></span> Menyimpan...';
        try {
            const res = await callAPI({ action: 'createBBMViolation', bulan, unit, tanggal_pengambilan: tglAmbil, tanggal_pengembalian: document.getElementById('bbm-viol-tgl-kembali')?.value.trim() || '-', nomor_kendaraan: document.getElementById('bbm-viol-no-kendaraan')?.value.trim() || '-' });
    ...
    window.bbmOpenEditViol = (idx) => {
        ...
        document.getElementById('bbm-ev-bulan').value = v.bulan || '';
        ...
        document.getElementById('bbm-ev-tgl-ambil').value = v.tanggal_pengambilan || '';
        document.getElementById('bbm-ev-tgl-kembali').value = v.tanggal_pengembalian || '';
        document.getElementById('bbm-ev-no-kendaraan').value = v.nomor_kendaraan || '';
    ...
    window.bbmSubmitEditViol = async () => {
        ...
        const orig = btn.innerHTML; btn.disabled = true; btn.innerHTML = '<span class="spinner spinner-sm"></span> Menyimpan...';
        try {
            const res = await callAPI({ action: 'updateBBMViolation', id: document.getElementById('bbm-ev-id').value, bulan: document.getElementById('bbm-ev-bulan-hidden').value, unit: document.getElementById('bbm-ev-unit-hidden').value, tanggal_pengambilan: document.getElementById('bbm-ev-tgl-ambil').value, tanggal_pengembalian: document.getElementById('bbm-ev-tgl-kembali').value, nomor_kendaraan: document.getElementById('bbm-ev-no-kendaraan').value });
            if (res?.success) { if (window.showToast) showToast('Catatan berhasil diperbarui!', 'success'); closeModal('bbm-editViolModal'); clearCache(); await loadViolations(true); }
            else if (window.showToast) showToast(res?.message || 'Gagal', 'error');
        } catch (e) { if (window.showToast) showToast('Error: ' + e.message, 'error'); }
        finally { btn.disabled = false; btn.innerHTML = orig; }
    };
    ...
    window.bbmExportViol = async () => {
        if (!confirm('Export catatan keterlambatan ke sheet baru?')) return;
        if (window.showToast) showToast('Mengekspor...', 'success');
        try {
            const res = await callAPI({ action: 'exportBBMViolationsReport', bulan: document.getElementById('bbm-filter-bulan-viol')?.value || '', unit: document.getElementById('bbm-filter-unit-viol')?.value || '' });
    ...
    function renderScoresUI() {
        const sv = document.getElementById('bbm-sanksi-value');
        if (sv) sv.textContent = allScores.sanksi !== undefined ? `${allScores.sanksi} poin per keterlambatan` : '-';
        const el = document.getElementById('bbm-filter-score-bulan');
        if (el && !el.value) {
            const mn = ['', 'JANUARI', 'FEBRUARI', 'MARET', 'APRIL', 'MEI', 'JUNI', 'JULI', 'AGUSTUS', 'SEPTEMBER', 'OKTOBER', 'NOVEMBER', 'DESEMBER'];
            el.value = mn[new Date().getMonth() + 1];
        }
        const bulan = el ? el.value : '';
        const container = document.getElementById('bbm-scores-container');
        if (!container) return;
        ...
        if (!allScores.scores || !allScores.scores.length) { container.innerHTML = '<div class="empty-state"><div class="empty-state-icon">📭</div><p>Tidak ada data penilaian</p></div>'; return; }
        ...
        container.innerHTML = '<div class="scores-grid">' + monthScores.map(score => {
            ...
            return `<div class="score-card">
                ...
                <div class="score-details">
                    ...
                    <div class="score-detail-item"><div class="score-detail-label">Keterlambatan</div><div class="score-detail-value">${score.jumlahPelanggaran || 0}</div></div>
...
<div class="container">
    <div class="section-page-header">
        <h1 class="section-page-title">Pelayanan Voucher BBM</h1>
        <p class="section-page-subtitle">Kelola permintaan, catatan keterlambatan, dan penilaian penggunaan voucher BBM</p>
    </div>

    <div class="tabs">
        <button class="tab active" onclick="bbmSwitchTab('requests',event)">Permintaan Voucher</button>
        <button class="tab" onclick="bbmSwitchTab('violations',event)">Catatan Keterlambatan</button>
        <button class="tab" onclick="bbmSwitchTab('scores',event)">Penilaian</button>
    </div>
    <div class="tabs-dropdown">
        <select onchange="bbmSwitchTabDropdown(this.value)">
            <option value="requests">Permintaan Voucher</option>
            <option value="violations">Catatan Keterlambatan</option>
        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="filter-container">
                    ...
                    <button onclick="bbmLoadRequests(true)" class="btn btn-sm btn-action-view" title="Refresh">
    ...
    <div id="bbm-tab-violations" class="tab-content">
        <div class="card">
            <div class="card-header">
                ...
                <div class="filter-container">
                    <select class="select-input" id="bbm-filter-bulan-viol" onchange="bbmApplyViolFilter()">
                    ...
                    <select class="select-input" id="bbm-filter-unit-viol" onchange="bbmApplyViolFilter()">
    ...
    <div id="bbm-tab-scores" class="tab-content">
        <div class="card">
            <div class="card-header">
                ...
                <div class="filter-container">
                    <select class="select-input" id="bbm-filter-score-bulan" onchange="bbmLoadScores(false)">
...
<div id="bbm-approveModal" class="modal-overlay" onclick="if(event.target===this)this.style.display='none'">
    <div class="modal">
        ...
        <div class="modal-content">
            <div class="alert alert-info">Status akan diubah menjadi <strong>Disetujui</strong>.</div>
        ...
        <div class="modal-footer">
            ...
            <button onclick="bbmConfirmApprove()" class="btn btn-action-approve" style="flex:1;" id="bbm-confirm-approve-btn">${ICONS.check} Setujui</button>
        </div>
    </div>
</div>
...
<div id="bbm-editModal" class="modal-overlay" onclick="if(event.target===this)this.style.display='none'">
    <div class="modal">
        ...
        <div class="modal-footer">
            ...
            <button onclick="bbmSubmitEdit()" class="btn btn-action-edit" style="flex:1;" id="bbm-submit-edit-btn">Simpan Perubahan</button>
        </div>
    </div>
</div>
...
<div id="bbm-addViolModal" class="modal-overlay" onclick="if(event.target===this)this.style.display='none'">
    <div class="modal">
        <div class="modal-header"><h2 class="modal-title">Tambah Catatan Keterlambatan BBM</h2></div>
        <div class="modal-content">
            <div class="form-group">
                <label class="input-label">Bulan *</label>
                <select class="form-input" id="bbm-viol-bulan" required>
                    <option value="">Pilih Bulan</option>
            ...
            <div class="form-group">
                ...
                <select class="form-input" id="bbm-viol-unit" required>
            ...
            <div class="form-group"><label class="input-label">Tanggal Pengambilan *</label><input type="text" class="form-input" id="bbm-viol-tgl-ambil" placeholder="Contoh: 07-01-2026" required></div>
            <div class="form-group"><label class="input-label">Tanggal Pengembalian</label><input type="text" class="form-input" id="bbm-viol-tgl-kembali" placeholder="Kosongkan jika belum kembali"></div>
            <div class="form-group"><label class="input-label">Nomor Kendaraan</label><input type="text" class="form-input" id="bbm-viol-no-kendaraan" placeholder="Contoh: AB 1234 CD"></div>
        </div>
        <div class="modal-footer">
            <button onclick="document.getElementById('bbm-addViolModal').style.display='none'" class="btn" style="flex:1;">Batal</button>
            <button onclick="bbmSubmitViol()" class="btn btn-action-edit" style="flex:1;" id="bbm-submit-viol-btn">Simpan</button>
        </div>
    </div>
</div>
...
<div id="bbm-editViolModal" class="modal-overlay" onclick="if(event.target===this)this.style.display='none'">
    <div class="modal">
        <div class="modal-header"><h2 class="modal-title">Edit Catatan Keterlambatan</h2></div>
        <div class="modal-content">
            <div class="info-box">
                <p class="info-box-label">Info (tidak dapat diubah)</p>
                <div style="display:flex;gap:12px;">
                    <div style="flex:1;"><label class="input-label">Bulan</label><input type="text" class="form-input" id="bbm-ev-bulan" readonly style="background:#f0f4f8;cursor:not-allowed;"></div>
                    <div style="flex:1;"><label class="input-label">Unit</label><input type="text" class="form-input" id="bbm-ev-unit" readonly style="background:#f0f4f8;cursor:not-allowed;"></div>
                </div>
            </div>
            <div class="form-group" style="margin-top:16px;"><label class="input-label">Tanggal Pengambilan</label><input type="text" class="form-input" id="bbm-ev-tgl-ambil" placeholder="Contoh: 07-01-2026"></div>
            <div class="form-group"><label class="input-label">Tanggal Pengembalian</label><input type="text" class="form-input" id="bbm-ev-tgl-kembali" placeholder="Contoh: 10-01-2026"></div>
            <div class="form-group"><label class="input-label">Nomor Kendaraan</label><input type="text" class="form-input" id="bbm-ev-no-kendaraan"></div>
        ...
        <div class="modal-footer">
            ...
            <button onclick="bbmSubmitEditViol()" class="btn btn-action-edit" style="flex:1;" id="bbm-submit-ev-btn">Simpan Perubahan</button>
        </div>
    </div>
</div>`;
        loadRequests();
